...
public class ReflectionUtil {
	...
	/**
	 * 
	 * Для некоторого класса определяет каким классом был параметризован один из
	 * его предков с generic-параметрами.
	 * 
	 * 
	 * 
	 * @param actualClass
	 ...
	 * 
	 * @param genericClass
	 *            класс, для которого определяется значение параметра
	 * 
	...
	public static Class getGenericParameterClass(final Class actualClass,
		...
		while (true) {
			...
			Type rawType = isParameterizedType ? ((ParameterizedType) genericSuperclass)
		...
		if (result == null) {
			// Should never happen. :)
			throw new IllegalStateException(
					"Unable to determine actual parameter type for "
							+ actualClass.getName() + ".");
		}
		if (!(result instanceof Class)) {
			// Похоже, что параметр - массив или что-то еще, что не является
			// классом.
			throw new IllegalStateException("Actual parameter type for "
					+ actualClass.getName() + " is not a Class.");
		}
		return (Class) result;
	}

	public static int getParameterTypeDeclarationIndex(
		...
		if (actualArgumentIndex != null) {
			...
			throw new IllegalStateException("Argument "
					+ typeVariable.toString() + " is not found in "
